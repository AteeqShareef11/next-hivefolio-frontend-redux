...
const CardCommunity = ({ id, image_profile,email, name, username, games, community, full, responsive, list }) => {

    const allCommunities = useSelector(state => state.allData.communities);

    let foundCommunity = null;
    if(allCommunities && allCommunities.data !== undefined && community && community !== undefined) {
        const matchCommunity = Array.isArray(allCommunities.data) ? allCommunities.data?.find((person) => {
        return person?.id === community?.id;
        }) : {}
    ...
    return (
        <div>
            ...
            {full === true && (
                        ...
                        <div>
                            {image_profile && image_profile.data!==null ? (
                                <div
                                    ...
                                    style={{
                                        backgroundImage: `url(${image_profile && image_profile.data.attributes!==null && image_profile.data.attributes.url})`
                                    }}
                ...
                {foundCommunity.attributes.members.data?.sort(() => Math.random() - 0.5).slice(0, 8)?.map(member => (
                    <img
                    ...
                    style={{
                        backgroundImage: `url(${member.attributes.image_profile && member.attributes.image_profile.data.attributes.url})`
                    }}
                    alt=""
                />
                ))}
                ...
                {(foundCommunity && Array.isArray(foundCommunity.attributes.games.data) && foundCommunity.attributes.games.data.length > 0) && (
                    <div
                    className="items-end rounded-3xl mt-2 p-1 body-small text-xs bg-dark text-white"
                    >
                    {foundCommunity.attributes.games.data?.sort(() => Math.random() - 0.5).slice(0, 1)?.map((item) => item.attributes.name)}
                    </div>
                )}
            </IonCardContent>
            )}
            ...
            {(foundCommunity && foundCommunity!==null && Array.isArray(foundCommunity.attributes.games.data) && foundCommunity.attributes.games.data.length > 0) && (
                <div
                className="items-end rounded-3xl mt-2 p-1 body-small text-xs bg-dark text-white"
                >
                {foundCommunity.attributes.games.data?.sort(() => Math.random() - 0.5).slice(0, 1)?.map((item) => item.attributes.name)}
