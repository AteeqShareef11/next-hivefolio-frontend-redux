...
const CardUserModal2 = ({ id, teamId, username, image_profile, email, gamertag, team }) => {
    ...
    useEffect(()=>{
        ...
        Axios.get(`https://hivefolio.herokuapp.com/api/teams/${teamId}`) 
        .then(data => {
    ...
    // Searches through the team member list for a matching member id
    const existingUser = team["members"].find(x => x.id == id);
    ...
    if(existingUser){
       ...
       var config = {
           ...
           method: "PUT",
           url: `https://hivefolio.herokuapp.com/api/teams/${teamId}`,
           headers:{
            'content-type': 'application/json',
            Authorization: `Bearer ${localStorage.getItem('token')}`,
           },
           data: {...team,members: team.members.filter(item => item.id !== id).map(item => item.id)}
       }
       Axios(config)
       .then(response =>{
                            ...
                            >
                                ...
                                <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                                    ...
                                    <IonToast
                                        ...
                                        message={`User has been removed from team ${id.name}`}
                                        duration={1000}
                                    />
                                </div>
                                </div>
