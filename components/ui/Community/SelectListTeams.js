...
/* Components */
import CardTeamListAddCommunity from './CardTeamListAddCommunity';
...
const getUsersData = async key => {
  ...
  const resTeams = await fetch(
    `https://hivefolio.herokuapp.com/api/teams?populate=*&_limit=4&_start=${page * 4}`
  );
