...
import { fetchCommunityId, communityAddMember } from '../../redux/actions/communityActions';
import { createNotificationCommunityMemberRequest } from '../../redux/actions/notificationActions';


const CardUserListAddCommunity = ({
  userId,
  username,
  image_profile,
  email,
  ...
  const [communityDataMember, setCommunityDataMember] = useState({});
  ...
  useEffect(() => {
    Axios.get(`https://hivefolio.herokuapp.com/api/communities/${id}`)
      .then(data => {
        setCommunityDataMember(data.data.members);
      })
      .catch(err => {
        console.log('ERR: ', err);
      });
  ...
  const handleCommunityMemberRequest = async (event) => {
  ...
  const handleAddUser = async (event) => {
    //event.preventDefault()
    dispatch(communityAddMember(id, userId, users, communityDataMember ))
  }
                  ...
                  <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                    ...
                    <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                      <button
                        ...
                        onClick={() => {
                          handleCommunityMemberRequest(userId);
                          setShowToast1(true);
                        }}
                      >
                        {error && <p>{error}</p>}
                      ...
                      <IonToast
                        ...
                        /* message={`User has been added to community ${community.name}`} */
                        duration={1000}
                      />
                    </div>
                  </div>
...
export default CardUserListAddCommunity;
