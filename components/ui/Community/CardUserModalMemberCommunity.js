...
import { fetchCommunityId, communityRemoveMember, communityAddAdmin, communityRemoveAdmin } from '../../redux/actions/communityActions';

const CardUserModalMemberCommunity = ({
  userId,
  username,
  image_profile,
  email,
  ...
  const [communityDataAdmin, setCommunityDataAdmin] = useState({});
  ...
  useEffect(() => {
    Axios.get(`https://hivefolio.herokuapp.com/api/communities/${id}`)
      .then(data => {
        ...
        setCommunityDataAdmin(data.data.admins);
      })
      .catch(err => {
        console.log('ERR: ', err);
      });
  ...
  let adminId = community.admins?.find((person) => {
  ...
  const handleAddAdmin = async (event) => {
    ...
    dispatch(communityAddAdmin(id, userId, users, user, community, communityDataAdmin ))
  }

  /* Remove user to admin list */
  const handleRemoveAdmin = async (event) => {
    //event.preventDefault()
    dispatch(communityRemoveAdmin(id, userId, community ))
  }

  /* Remove user */
  const handleRemoveUser = async (event) => {
    //event.preventDefault()
    dispatch(communityRemoveMember(id, userId, community ))
  }
                  ...
                  <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                    <div className="sm:flex sm:items-start">
                      ...
                      <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left mb-4">
                        ...
                        <div className="mt-2">
                          ...
                          {(community.admins?.length <= 1 && adminId) ? (
                            ...
                            {/* <li className="py-4 flex items-center justify-between">
                              ...
                              <button
                                type="button"
                                className="bg-gray-200 ml-4 relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-light-blue-500"
                                aria-pressed="true"
                    ...
                    <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                      ...
                      <IonToast
                        ...
                        message={`User has been removed from community ${id.name}`}
                        duration={1000}
                      />
                    </div>
                  </div>
...
export default CardUserModalMemberCommunity;
