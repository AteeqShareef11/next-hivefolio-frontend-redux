...
import { communityRemoveTeam } from '../../redux/actions/communityActions';

const CardUserModalTeamCommunity = ({ teamId, image_profile, email, name }) => {
  ...
  useEffect(() => {
    Axios.get(`https://hivefolio.herokuapp.com/api/communities/${id}`)
      .then(data => {
        setCommunityData(data.data.teams);
      })
      .catch(err => {
        console.log('ERR: ', err);
      });
...
/* const adminId = community.admins?.find((person) => {
  ...
  const handleRemoveTeam = async (event) => {
    ...
    dispatch(communityRemoveTeam(id, teamId, community ))
  }
        ...
        <div class="flex justify-between items-center w-full">
          <div className="flex flex-row">
            <div class="flex-shrink-0 h-10 ">
              {image_profile ? (
                <div
                  ...
                  alt={`Profile name ${community.name}`}
                >
            ...
            <div class="ml-4">
              ...
              {/* <div class="text-sm text-gray-500">{community.name}</div> */}
            </div>
          </div>
                      ...
                      {/* <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left mb-4">
                        ...
                        <div className="mt-2 mb-8">
                          ...
                          <p className="-mt-8 text-sm text-gray-500">
                            Would you like to add {name} to community {community.name}?
                          </p>
                        </div>
...
export default CardUserModalTeamCommunity;
