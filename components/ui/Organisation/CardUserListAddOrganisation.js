import { IonButton, IonCardContent, IonInput, IonItem, IonLabel, IonModal, IonToast } from '@ionic/react';
...
import { fetchOganisationId, organisationAddMember, removeSelectedOrganisation } from '../../redux/actions/organisationActions';
import { createNotificationOrganisationMemberRequest } from '../../redux/actions/notificationActions';


const CardUserListAddOrganisation = ({
  userId,
  username,
  image_profile,
  email,
  ...
  const [organisationDataMember, setOrganisationDataMember] = useState({});
  ...
  useEffect(() => {
    Axios.get(`https://hivefolio.herokuapp.com/api/organisations/${id}`)
      .then(data => {
        setOrganisationDataMember(data.data.members);
  ...
  const handleAddUser = async (event) => {
    //event.preventDefault()
    dispatch(organisationAddMember(id, userId, users, organisationDataMember ))
  }
          ...
          <IonModal
            ...
            <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
              <IonToast
                ...
                message={`User has been added to organisation ${name}`}
                duration={1000}
              />
            </div>
...
export default CardUserListAddOrganisation;
