import { IonButton, IonCardContent, IonItem, IonModal, IonToast } from '@ionic/react';
...
import { fetchOganisationId, organisationRemoveMember, organisationAddAdmin, organisationRemoveAdmin} from '../../redux/actions/organisationActions';

const CardUserModalMemberOrganisation = ({ userId, username, image_profile, email, gamertag,  setShowLoading }) => {
  const { id } = useParams();
  ...
  const [organisationData, setOragnisationData] = useState({});
  const [organisationDataMember, setOragnisationDataMember] = useState({});
  const [organisationDataAdmin, setOragnisationDataAdmin] = useState({});
  const [organisationDataCoach, setOragnisationDataCoach] = useState({});
  const [organisationDataCaptain, setOragnisationDataCaptain] = useState({});
  const [error, setError] = useState('');
  const [showToast1, setShowToast1] = useState(false);
  //const [update, setUpdate ] = useState(false);

  useEffect(() => {
    Axios.get(`https://hivefolio.herokuapp.com/api/organisations/${id}`)
      .then(data => {
        setOragnisationData(data.data);
        setOragnisationDataMember(data.data.members);
        setOragnisationDataAdmin(data.data.admins);
        console.log("data.data.admins", data.data.admins)
      })
      .catch(err => {
...
let adminId = organisation.admins?.find((person) => {
  return person.id === userId;
})

/* New Add user to admins list */
const handleAddAdmin = async (event) => {
  //event.preventDefault()
  dispatch(organisationAddAdmin(id, userId, users, user, organisation, organisationDataAdmin ))
}

/* New Remove user to admin list */
const handleRemoveAdmin = async (event) => {
  //event.preventDefault()
  dispatch(organisationRemoveAdmin(id, userId, organisation ))
}
...
const handleRemoveUser = async (event) => {
  ...
  /* const handleRemoveUser = async event => {
    ...
    // Searches through the organisation member list for a matching member id
    const existingUser = organisation['members']?.find(x => x.id == userId);
    const existingAdmin = organisation['admins']?.find(x => x.id == userId);

    if (existingUser, existingAdmin) {
      var config = {
        method: 'PUT',
        url: `https://hivefolio.herokuapp.com/api/organisations/${id}`,
        headers: {
        ...
        <div class="flex justify-between items-center w-full">
            ...
            <div className="sm:items-start m-8">
              ...
              {/* <div className="mt-2 mb-8">
                ...
                <p className="-mt-8 text-sm text-gray-500">
                  Would you like to add {gamertag} to team {organisation.name}?
                </p>
              </div> */}
                ...
                {/* Admins */}
                {(organisation.admins?.length <= 1 && adminId) ? (
          ...
          <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
            ...
            <IonToast
              ...
              message={`User has been added to organisation ${organisation.name}`}
              duration={1000}
            />
              </div>
            </div>
                    ...
                    <div className="sm:flex sm:items-start">
                      ...
                      <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left mb-4">
                        ...
                        <div className="mt-2 mb-8">
                          <h3 className="">{gamertag}</h3>
                          <p className="-mt-8 text-sm text-gray-500">
                            Would you like to add {gamertag} to organisation {organisation.name}?
                          </p>
                        </div>
...
export default CardUserModalMemberOrganisation;
