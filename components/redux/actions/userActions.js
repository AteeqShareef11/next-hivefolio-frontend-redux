...
export const editUser = (formData, id) => {
    return async (dispatch) => {
      try {
        ...
        const response = await hivefolioApi.put(`/users/${id}`, formData, {headers: {
  ...
  export const addGame = (id, user, games, gameId) => {
    ...
    if (!isExistingGame) {
      ...
      // add it to members
      const newGames = [...user.games, game]
  ...
  export const addUserImage = (setShowLoading, request, id) => {
    ...
    return async (dispatch) => {
      try {
        ...
        const response = await hivefolioApi.put(`/users/${id}`, request, {headers: {
          // 'content-type': 'multipart/form-data',
          "Accept": ["multipart/form-data", "Application/json"]
            }
        })
