import { 
  ...
  const AccountStreams = ({match}) => {
    ...
    const {stream_twitch} = loggedInUser;

    const [formValues, setFormValues] = useState({
      stream_twitch: loggedInUser?.stream_twitch || "",
    });

    useEffect(() => {
      setFormValues({...formValues, stream_twitch})
    ...
    const handleEditSubmit = async (event) => {
      ...
      formData.append('data', JSON.stringify({
        stream_twitch: formValues.stream_twitch,
      }));
      ...
      dispatch(editUser(body, id, loggedInUser))
 ...
 */
      ...
      return (
          <IonPage >
        <IonHeader >
          <title>Account Streams - {user.username}</title>
                            ...
                            <section className="col-span-2">
                              <div className="px-4 py-5 grid xs:grid-cols-1 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-1 xl:grid-cols-1 gap-6">
                                <IonItem className="mb-4 rounded-lg">
                                  ...
                                  <IonInput
                                    value={formValues.stream_twitch}
                                    name="stream_twitch" 
                                ...
                                <div className="">
                                  {loggedInUser.stream_twitch && (
                                    <CardTwitch
                                    twitchId={loggedInUser.stream_twitch}
                                    />
                                  )}
                                </div>
                              </div>
