...
import Onboarding01 from '../ui/Onboarding/Onboarding01';
import Onboarding02 from '../ui/Onboarding/Onboarding02';
import Onboarding03 from '../ui/Onboarding/Onboarding03';
...
import { fetchGames, fetchCharacters, fetchUserId, fetchUsers, removeSelectedUser } from '../redux/actions/coreActions';
...
const Onboarding = () => {
  ...
  console.log("AboveloggedInUser==========>>", loggedInUser)
  // console.log("loggedInUser onboarding.js state", loggedInUser.onboarding)
  ...
  const steps = [
    ...
    { component: Onboarding01, label: "Sign Up" },
    { component: Complete, label: "Complete" },
    { component: Reset, label: "Reset" }
  ]

  /* if(loggedInUser && loggedInUser.onboarding !== undefined && loggedInUser.onboarding !== false) {
  ...
  useEffect(() => {
    ...
    if ((user !== undefined && user.id !== loggedInUser.id)) {
      dispatch(fetchUserId(user.id))
      setShowLoading(true)
    } else {
              ...
              <div className='flex h-full mt-8 text-center w-full max-w-xl xs:hidden sm:hidden md:hidden lg:block xl:block'>
                {page === 0 && (
                  <div className="flex w-full space-y-5 md:pr-16 xs:hidden sm:hidden md:hidden lg:block xl:block items-start">
                    ...
                    <h3 class="mb-6">{user.gamertag}</h3>
                      ...
                      {loggedInUser.type_user && (
                        <div className="">
                          <CardGrid
                            ...
                            className="mt-8"
                          >
                            {loggedInUser.type_user.sort(() => Math.random() - 0.5).map(type => (
                              <div
