...
import { fetchTeamId, editTeam, addGameToTeam, removeGameFromTeam, addTypeteam, removeTypeteam, fetchTypeteams } from '../redux/actions/coreActions';
import { fetchGames } from '../redux/actions/gameActions';
//import { fetchTypeteams } from '../redux/actions/typeActions';
...
const EditTeam = (  ) => {
  ...
  const {name, username, introduction, tagline, admins} = selectedTeam;

  const [formValues, setFormValues] = useState({
    name: selectedTeam?.name || "",
    username: selectedTeam?.username || "",
    introduction: selectedTeam?.introduction || "",
    tagline: selectedTeam?.tagline || "",
  })

  const [showToast1, setShowToast1] = useState(false);
  const [showSubmitFormToaster, setShowSubmitFormToaster] = useState(false);
  ...
  const handleEditSubmit = async (event) => {
    ...
    dispatch(editTeam(body, id, setShowSubmitFormToaster))
...
const handleRemoveTypeTeam = async (event, removeId) => {
  ...
  return (
    <IonPage>
      <IonHeader>
        <title>Edit Team - {username}</title>
      ...
      <IonContent fullscreen className="">
        <div className="max-width">
          ...
          {(!adminId) && (history.push(`/team/${username}`))}
                    ...
                    <div className="pt-6 divide-y divide-gray-200">
                      ...
                      <div className="mt-4 py-4 px-4 flex justify-end sm:px-6">
                        ...
                        <button
                          ...
                          //onClick={() => setShowSubmitFormToaster(true)}
                          expand="block"
                        >
                          Save
                        </button>
                        <IonToast
                          isOpen={showSubmitFormToaster}
                          onDidDismiss={() => setShowSubmitFormToaster(false)}
